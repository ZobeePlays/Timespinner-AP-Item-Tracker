...
timespinner_location = {}
timespinner_location.__index = timespinner_location
...
function timespinner_location.new(name)
    local self = setmetatable({}, timespinner_location)
    if name then
...
function timespinner_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = timespinner_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function timespinner_location:connect_two_ways(exit, rule)
...
function timespinner_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function timespinner_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function timespinner_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function timespinner_location:accessibility()
...
-- 
function timespinner_location:discover(accessibility, keys)
...
entry_point = timespinner_location.new("entry_point")
